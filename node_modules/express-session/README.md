...
#### Options
  ...
  - `key` - cookie name defaulting to `connect.sid`.                            // Cookie  名称
  - `store` - session store instance.                                           // 存储实例
  - `secret` - session cookie is signed with this secret to prevent tampering.  // 加密密钥（防止篡改）
